...
static bool setSizeAndSubtype(videoDevice * VD, int attemptWidth, int attemptHeight, GUID mediatype){
	...
	if (VD->requestedFrameTime != -1){
		...
		printf("#set###pVih->AvgTimePerFrame [%ld]", pVih->AvgTimePerFrame);
	}
...
/*
DirectShow中枚举摄像头的采集分辨率 \
http://blog.sina.com.cn/s/blog_48720e900100rs9v.html
*/
using namespace std;
/* https://raw.githubusercontent.com/hkimata/directshow/master/common/smartptr.h */
...
/*
10帧、20帧、25帧和30帧。DirectShow中是以100纳秒为单位，也就是得到的实际值是1000000、500000、4000000和3333333
...
*/
HRESULT GetCaptureFps(IBaseFilter* pCapFilter, ICaptureGraphBuilder2* pBuild) {
	HRESULT hr;
	SmartPtr<IAMStreamConfig> pam;
	hr = pBuild->FindInterface(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video,
		pCapFilter, IID_IAMStreamConfig, reinterpret_cast<void**>(&pam)); // 得到媒体控制接口
	...
	int nSize = 0;
	hr = pam->GetNumberOfCapabilities(&nCount, &nSize);
	ofstream outfile("captureFPS.txt"); // 准备写入文件
		// 判断是否为视频信息
		if (sizeof(VIDEO_STREAM_CONFIG_CAPS) == nSize) {
			for (int i = 0; i < nCount; i++) {
				VIDEO_STREAM_CONFIG_CAPS scc;
				...
				if (pmmt->formattype == FORMAT_VideoInfo) {
					...
					int fps=10000000/ nFrame;
					outfile << fps << endl; // 写入文件
				}
			}
		}
	outfile.close(); // 关闭文件
...
HRESULT GetCaptureInfo(IBaseFilter* pCapFilter, ICaptureGraphBuilder2* pBuild) {
	HRESULT hr;
	SmartPtr<IAMStreamConfig> pam;
	hr = pBuild->FindInterface(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video,
		pCapFilter, IID_IAMStreamConfig, reinterpret_cast<void**>(&pam)); // 得到媒体控制接口
	...
	ofstream outfile("captureInfo.txt"); // 准备写入文件
    outfile <<"Capability count: "<<nCount<<endl;
	// 判断是否为视频信息
	if (sizeof(VIDEO_STREAM_CONFIG_CAPS) == nSize) {
		for (int i = 0; i < nCount; i++) {
			VIDEO_STREAM_CONFIG_CAPS scc;
			...
			if (pmmt->formattype == FORMAT_VideoInfo) {
				...
				int nFrame = pvih->AvgTimePerFrame; // 得到采集的帧率
				int fps=10000000/ nFrame;
				outfile << nwid << " * " << nhei <<"  "<< fps << endl; // 写入文件
			}
		}
	}
...
int videoInput::start(int deviceID, videoDevice *VD){
	...
	/**begin***/
	//GetCaptureRatio(VD->pVideoInputFilter, VD->pCaptureGraph);
    //GetCaptureFps(VD->pVideoInputFilter, VD->pCaptureGraph);
    GetCaptureInfo(VD->pVideoInputFilter, VD->pCaptureGraph);
	/**end***/
